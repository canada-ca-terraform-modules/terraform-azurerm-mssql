module "sqlserver" {
  source = "git::https://gitlab.k8s.cloud.statcan.ca/managed-databases/terraform-azurerm-mssql-server.git?ref=development"
...
module "elasticpool" {
  ...
  count = var.elasticpools == null ? 0 :length(var.elasticpools)
